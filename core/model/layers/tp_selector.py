...
class TPSelector(nn.Module):
    def __init__(self,
        ...
        self.tp_dimension = nn.Sequential(
            nn.Linear(2, self.before_concat_dim),
            nn.ReLU(),
        )
        
        self.prob_mlp = nn.Sequential(
            MLP(in_channels + self.before_concat_dim, hidden_dim, hidden_dim),
            nn.Dropout(p=0.1),
            nn.Linear(hidden_dim, 1)
        )


    def forward(self, feat_in: torch.Tensor, tp_candidate: torch.Tensor, candidate_mask=None):
        """
        ...
        batch_size, n, _ = tp_candidate.size()


        tp_candidate_feature = self.tp_dimension(tp_candidate)
        # tp_candidate_feature = tp_candidate
        
        feat_in_repeat = torch.cat([feat_in.repeat(1, n, 1), tp_candidate_feature], dim=2)
