...
class TargetReg(nn.Module):
    def __init__(self,
        ...
        self.norm = 'layer' # 'layer' # 'batch'

        self.offset_mlp = nn.Sequential(
            MLP(in_channels, hidden_dim, hidden_dim, norm=self.norm),
            # nn.Dropout(p=0.5),
            MLP(hidden_dim, hidden_dim, hidden_dim),
            nn.Linear(hidden_dim, 2)
        )

        self.yaw_pred = nn.Sequential(
            MLP(in_channels, hidden_dim, hidden_dim, norm=self.norm),
            # nn.Dropout(p=0.5),
            MLP(hidden_dim, hidden_dim, hidden_dim),
    ...
    def forward(self, feat_in: torch.Tensor, data):
        ...
        #     data.horizon[i] -= 7
        #     #print(i, horizon_sum, int(gt_dict['ttc'][i]))
        #     each_yaw = int(data.y_yaw[horizon_sum + int(data.horizon[i]) - 1].cpu())            
        #     each_y = data.y.reshape(-1, 2)[horizon_sum + int(data.horizon[i]) - 1].cpu().float()
        #     data_y_tensor[i] = data.y.reshape(-1, 2)[horizon_sum + int(data.horizon[i]) - 1]
        #     horizon_sum += int(data.horizon[i])
        #     data_yaw_list.append([each_yaw])
        # data.y_yaw = torch.from_numpy(np.array(data_yaw_list)).float().cuda()
        # data.y_yaw = data.y_yaw * np.pi / 180
        # print(data_y_tensor)        

        # compute probability for each candidate
        if self.norm == 'batch':
            feat_in = feat_in.squeeze(1)
        # print("offset", feat_in.shape)
        tar_offset_mean = self.offset_mlp(feat_in)                                         # [batch_size, n_tar, 2]
        # print("yaw")
        yaw_pred = self.yaw_pred(feat_in)
        if self.norm == 'batch':
            tar_offset_mean = tar_offset_mean.unsqueeze(1)
            yaw_pred = yaw_pred.unsqueeze(1)
        ...
        return tar_offset_mean, yaw_pred
    def loss(self,
