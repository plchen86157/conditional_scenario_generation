...
class VectorNetBackbone(nn.Module):
    ...
    def forward(self, data, nearest_subgraph):
        """
        args:
        ...
        subgraph_center_pos = avg_pool(data.cluster, data).x[:, :2].reshape(batch_size, -1, 2) # 128, 48, 2
        subgraph_fea = sub_graph_out.reshape(batch_size, -1, sub_graph_out.shape[-1]) # 128, 48, 64
        candidate_reshape = data.candidate.reshape(batch_size, data.candidate_len_max[0], 2) # 128, 525, 2
        candidate_reshape = candidate_reshape.unsqueeze(2)
        dist = torch.norm(candidate_reshape - subgraph_center_pos.view(batch_size, 1, subgraph_center_pos.shape[1], 2), dim=-1)
        _, nearest_indices = torch.topk(dist, nearest_subgraph, dim=2, largest=False)
        nearest_features = torch.gather(subgraph_fea.unsqueeze(2).expand(-1, -1, nearest_subgraph, -1).clone(), 
                                     1, nearest_indices.unsqueeze(-1).expand(-1, -1, -1, subgraph_fea.shape[-1])) #128, 525, 10, 64
        nearest_features = nearest_features.permute(0, 2, 1, 3)
        nearest_features = nearest_features.reshape(batch_size, data.candidate_len_max[0], -1) # 128, 525, 640
        ...
        # print("sub_graph_out:", sub_graph_out.shape) #6144, 64
        x = torch.cat([sub_graph_out, id_embedding], dim=1).view(batch_size, -1, self.subgraph.out_channels + 2) # 128, 48, 66
        valid_lens = data.valid_len
        # print("id_embedding:", id_embedding.shape, id_embedding) #6144,2
        # exit()
        if self.training:
            ...
            # print("x:", x.shape) # 128, 48, 66
            global_graph_out = self.global_graph(x, valid_lens=valid_lens)
            #print("global_graph_out:", global_graph_out.shape) # 128, 48, 64
            if self.with_aux:
                aux_in = global_graph_out.view(-1, self.global_graph_width)[mask_polyline_indices]
                aux_out = self.aux_mlp(aux_in)

                return global_graph_out, aux_out, aux_gt, nearest_features

            return global_graph_out, None, None, nearest_features
