...
class ArgoversePreprocessor(Preprocessor):
    ...
    def get_obj_feats(self, data):
        ...
        #print("ctr_line_candts_len:", len(ctr_line_candts)) #2~5 list
        #print("ctr_line_candts[0]_len:", len(ctr_line_candts[0])) #40 50...
        #print("ctr_line_candts:", ctr_line_candts.shape)
        #print("ctr_line_candts:", ctr_line_candts) #lines
        ...
        for i, _ in enumerate(ctr_line_candts):
            #print(ctr_line_candts[i] - orig.reshape(-1, 2))
            #print((ctr_line_candts[i] - orig.reshape(-1, 2)).T.shape)
            #print(rot.shape)
            #print(i, ctr_line_candts[i])
        ...
        #self.plot_target_candidates(ctr_line_candts, agt_traj_obs, agt_traj_fut, tar_candts)    # strange
    ...
    def get_lane_graph(self, data):
        ...
        for lane_id in lane_ids:
            ...
            #print("lane:", lane)
            centerline = np.matmul(data['rot'], (lane.centerline - data['orig'].reshape(-1, 2)).T).T #normalization
            #print("centerline:", centerline) #one line for 10 points
            x, y = centerline[:, 0], centerline[:, 1]
            if x.max() < x_min or x.min() > x_max or y.max() < y_min or y.min() > y_max: #filter out <-100 or >100
                continue
            else:
                ...
                lanes[lane_id] = lane
                #print("centerline:", lane.centerline)
                #print("polygon:", lane.polygon)
        #print("lanes:", lanes) #each lane id with lane segment
        lane_ids = list(lanes.keys())
        ctrs, feats, turn, control, intersect = [], [], [], [], []
        for lane_id in lane_ids:
            lane = lanes[lane_id] # lane here is lane segment
            ...
            #print("ctrln:", ctrln, "len:", len(ctrln))
            #print("ctrs:", ctrs)
            #print("feats:", feats)
            #print("direction:", lane.turn_direction)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-r", "--root", type=str, default="/home/carla/experiments/TNT-Trajectory-Predition/dataset")
    parser.add_argument("-d", "--dest", type=str, default="/home/carla/experiments/TNT-Trajectory-Predition/dataset")
