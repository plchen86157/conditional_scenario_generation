...
class TNTTrainer(Trainer):
    ...
    def test(self,
        ...
        with torch.no_grad():
            for data in tqdm(self.test_loader):
                ...
                if self.multi_gpu:
                    ...
                    pred_target, target_point_pred_pos, offset_pred, yaw_pred, pred_target_point, pred_target_point_yaw, tar_offset_pred, atr_yaw_pred, tp_index, RCNN_labels = self.model.inference(data.to(self.device))
                else:
                    pred_target, target_point_pred_pos, offset_pred, yaw_pred, pred_target_point, pred_target_point_yaw, tar_offset_pred, atr_yaw_pred, tp_index, RCNN_labels = self.model.inference(data.to(self.device))
                ...
                if not only_1_batch_flag:
                    ...
                    for s in range(batch_size):
                        ...
                        # exit()
                        candidate_pos = data.candidate.view(-1, n, 2).cpu()
                        candidate_pos = self.convert_coord(candidate_pos[s], origs[s], rots[s])
                        gt_index_tuple = np.where(candidate_gt[s] == 1)
                        gt_target_point = candidate_pos[gt_index_tuple, :]
                        pred_index_tuple = np.where(RCNN_labels[s] == 1)
                        pred_RCNN_target_point = candidate_pos[pred_index_tuple, :]
                        ...
                        positive_num = pred_RCNN_target_point.shape[1]
                        positive_num_list.append(positive_num)
                        if positive_num != 0:
                            if plot_RCNN_target:
                                for point_index in range(pred_RCNN_target_point.shape[1]):
        ...
        print("F1 score:", 2 * (precision * recall) / (precision + recall) if (precision + recall) != 0 else None)
    ...
    def calculate_F1(self, prediction, ground_truth):
        ...
        accuracy = true_positives / len(ground_truth_set)
