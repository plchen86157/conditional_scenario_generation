...
class TNTTrainer(Trainer):
    ...
    def test(self,
        ...
        with torch.no_grad():
            for data in tqdm(self.test_loader):
                ...
                for i in range(batch_size):
                    ...
                    each_yaw = float(data.y_yaw[horizon_sum + int(data.horizon[i]) - 1].cpu())
                ...
                for s in range(batch_size):
                    ...
                    sce_df = pd.read_csv('nuscenes_data/trajectory/' + split + '/' + seq_ids[s] + '.csv', index_col=0)
                ...
                if not flag:
                    ...
                    for s in range(batch_size):
                        ...
                        sce_df = pd.read_csv('nuscenes_data/trajectory/' + split + '/' + now_scenario + '.csv', index_col=0)
                        ...
                        attacker_gt_rec = [gt_trajectories[now_scenario][-1][0] + atr_pos_offset[s][0], gt_trajectories[now_scenario][-1][1] + atr_pos_offset[s][1], vehicle_width
                                    , vehicle_length, (gt_yaw_np[s] + atr_yaw_offset[s]) + (90.0) * np.pi / 180]
                        x_1 = float(np.cos(
                        ...
                        print("pred_target_point_yaw:", pred_target_point_yaw)
                        print("atr_yaw_pred:", atr_yaw_pred)
                        print("gt_yaw_np:", gt_yaw_np)
                        ####################
        ...
        print("Real collision rate:", attacker_right_collision_rate / all_data_num)
        ...
        print("lane change ratio:", lane_change_num / lane_change_all, " Real collision:", col_lane_change_num / lane_change_all)
        print("opposite direction ratio:", opposite_direction_num / opposite_direction_all, " Real collision:", col_opposite_direction_num / opposite_direction_all)
        print("rear end ratio:", rear_end_num / rear_end_all, " Real collision:", col_rear_end_num / rear_end_all)
        print("junction crossing ratio:", junction_crossing_num / junction_crossing_all, " Real collision:", col_junction_crossing_num / junction_crossing_all)
        print("LTAP:", LTAP_num / LTAP_all, " Real collision:", col_LTAP_num / LTAP_all)
