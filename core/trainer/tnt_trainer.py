...
class TNTTrainer(Trainer):
    ...
    def __init__(self,
        ...
        self.model = model_name(
            ...
            nearest_subgraph=nearest_subgraph,
        )
    ...
    def iteration(self, epoch, dataloader):
        ...
        for i, data in data_iter:
            ...
            if training:
                ...
                self.write_log("h_dist_loss",
                               loss_dict["h_dist_loss"].detach().item() / n_graph, i + epoch * len(dataloader))
                self.write_log("RCNN_cls_2module_loss",
                               loss_dict["RCNN_cls_2module_loss"] / n_graph, i + epoch * len(dataloader))
    ...
    def test(self,
        ...
        ego_iou_all_distribution, ttc_all_distribution, pred_right_lane_distribution = [], [], []
        ego_iou_lane_change, ttc_lane_change, pred_right_lane_LC = [], [], []
        ego_iou_junction_crossing, ttc_junction_crossing, pred_right_lane_JC = [], [], []
        ego_iou_LTAP, ttc_LTAP, pred_right_lane_LTAP = [], [], []
        ego_iou_opposite_direction, ttc_opposite_direction, pred_right_lane_OD = [], [], []
        ego_iou_rear_end, ttc_rear_end, pred_right_lane_RE = [], [], []
        ...
        with torch.no_grad():
            for data in tqdm(self.test_loader):
                ...
                candidate_pos_with_id = data.candidate_with_id.view(-1, n, 4).cpu()
                ...
                for s in range(batch_size):
                    ...
                    if seq_ids[s].split('_')[9] in keys:
                        gt_attacker_list.append([keys.index(seq_ids[s].split('_')[9])])
                    else:
                ...
                if not only_1_batch_flag:
                    ...
                    for s in range(batch_size):
                        ...
                        #    continue
                        # if now_scenario != 'data_hcis_v4.5_trainval_boston-seaport_LTAP_scene-0207_3-5-0.0-10_90.0_c930c758a6d94d41becc47266fd23988':
                        #     continue
                        # print(now_scenario)
                        ...
                        if plot_RCNN_target:
                            ...
                            for point_index in range(gt_target_point.shape[1]): #1,50,2
                                plt.scatter(gt_target_point[0][point_index][0], gt_target_point[0][point_index][1], s=7, c="blue", marker="s")
                        ...
                        if positive_num != 0:
                            ...
                            # check positive proposal
                            GT_lane_id = candidate_pos_with_id[s][indices_GT[s].cpu().numpy()][0][2]
                            positive_pred_lane_id = candidate_pos_with_id[s][pred_index_tuple[0][:]][:, 2]
                            positive_pred_lane_id_right = torch.sum(torch.eq(positive_pred_lane_id, GT_lane_id)).item()
                            pred_right_lane_ratio = positive_pred_lane_id_right / len(positive_pred_lane_id)
                            
                            # nusc_map = NuScenesMap(dataroot='./NuScenes/', map_name=now_scenario.split('_')[4])
                            # lanes = nusc_map.arcline_path
                            # for lane in lanes:
                            #     lane_record = nusc_map.get_arcline_path(lane)
                            # lane_record = nusc_map.get_arcline_path('5')
                            # lane_connectors = nusc_map.arcline_path
                            # opposite_lane_id = None
                            # for connector in lane_connectors:
                            #     if connector['id'] == 5:
                            #         opposite_lane_id = connector['reverse_lane_id']
                            #         break
                            all_TP += TP
                            all_FP += FP
                            all_FN += FN
                        ...
                        gt_record_yaw_distance = float(now_scenario.split('_')[8])
                        #attacker_id = now_scenario.split('.')[1].split('_')[-1]
                        attacker_id = now_scenario.split('_')[9]
                        right_guess_tp = 0
                        ...
                        if check_upper_bound_of_sim:
                            ...
                            pred_target_point[s][0][1] = gt_trajectories[now_scenario][-1][1]

                            pred_target_point_yaw[s][0][0] = gt_yaw_np[s] * np.pi / 180

                            tar_offset_pred[s][0] = torch.from_numpy(atr_pos_offset)[s][0]
                            tar_offset_pred[s][1] = torch.from_numpy(atr_pos_offset)[s][1]

                            att_yaw_pred_to_GT_flag = True

                            guess_attacker_id = attacker_id
                        ...
                        # data.cross[s] = - data.cross[s]
                        # print(atr_yaw_offset[s], data.cross[s])
                        if data.cross[s] < 0:
                        ...
                        # print(yaw_offset)
                        if att_yaw_pred_to_GT_flag:
                            yaw_offset = torch.from_numpy(atr_yaw_offset)[s].unsqueeze(0) * np.pi / 180
                            # print(yaw_offset)
                            attacker_yaw = pred_target_point_yaw[s][0][0] + yaw_offset + (90.0) * np.pi / 180
                            # attacker_yaw =  (gt_yaw_np[s] + 90.0 + atr_yaw_offset[s]) * np.pi / 180
                        else:
                            attacker_yaw = pred_target_point_yaw[s][0][0] + yaw_offset + (90.0) * np.pi / 180
                        attacker_pred_rec = [pred_target_point[s][0][0] + tar_offset_pred[s][0], pred_target_point[s][0][1] + tar_offset_pred[s][1], vehicle_width
                                    , vehicle_length, attacker_yaw]
                        ...
                        # tp_gv = math.sqrt((tp_gx - tp_start_x) ** 2 + (tp_gy - tp_start_y) ** 2) / min_t

                        # print("scenario length:", ttc)
                        ...
                        print(temp_yaw_dist * 180 / np.pi, now_scenario)
                        yaw_average_dist += temp_yaw_dist
                        if temp_yaw_dist * 180 / np.pi < 3:
                            temp_yaw_align = True
                            yaw_align_num += 1
                        else:
                            temp_yaw_align = False
                        perfect_quintic_check = False
                        if tp_gx == tp_pos[-1][0] and tp_gy == tp_pos[-1][1] and temp_yaw_align:
                            perfect_quintic_check = True
                            perfect_quintic_num += 1
                        # print("x:", tp_gx.cpu().numpy(), tp_pos[-1][0], "yaw:", tp_gyaw)
                        # print(perfect_quintic_check)
                                        ...
                                        for track_id, remain_df in sce_df.groupby("TRACK_ID"):
                                            if track_id == attacker_id:
                                                for i in range(frame_num):
                                                    ...
                                                    if is_intersection(x, y, town_name=now_scenario.split('_')[4]):
                                                        continue
                        ...
                        if not attacker_right_flag:
                            ### attacker IOU ###
                            ego_iou_all_distribution.append(now_iou)
                            ttc_all_distribution.append(ttc)
                            pred_right_lane_distribution.append(pred_right_lane_ratio)
                            if condition == 'LTAP':
                                ego_iou_LTAP.append(now_iou)
                                ttc_LTAP.append(ttc)
                                pred_right_lane_LTAP.append(pred_right_lane_ratio)
                                if attacker_iou > 0.5:
                                    attacker_IOU50 += 1
                                    ltap_IOU50 += 1
                            elif condition == 'JC':
                                ego_iou_junction_crossing.append(now_iou)
                                ttc_junction_crossing.append(ttc)
                                pred_right_lane_JC.append(pred_right_lane_ratio)
                                if attacker_iou > 0.5:
                                    attacker_IOU50 += 1
                                    junction_crossing_IOU50 += 1
                            elif condition == 'HO':
                                ego_iou_opposite_direction.append(now_iou)
                                ttc_opposite_direction.append(ttc)
                                pred_right_lane_OD.append(pred_right_lane_ratio)
                                if attacker_iou > 0.5:
                                    attacker_IOU50 += 1
                                    opposite_direction_IOU50 += 1
                            elif condition == 'RE':
                                ego_iou_rear_end.append(now_iou)
                                ttc_rear_end.append(ttc)
                                pred_right_lane_RE.append(pred_right_lane_ratio)
                                if attacker_iou > 0.5:
                                    attacker_IOU50 += 1
                                    rear_end_IOU50 += 1
                            elif condition == 'LC':
                                ego_iou_lane_change.append(now_iou)
                                ttc_lane_change.append(ttc)
                                pred_right_lane_LC.append(pred_right_lane_ratio)
                                if attacker_iou > 0.5:
                                    attacker_IOU50 += 1
                                    lane_change_IOU50 += 1
                        ...
                        if attacker_right_flag:
                            ...
                            title = 'Collide GT: ' + attacker_right
                        
                        # title = 'EGO: ' + str(pred_target_point[s][0]) + str(pred_target_point_yaw[s][0][0]) + 'ATT: ' + str(pred_target_point[s][0][0] + tar_offset_pred[s][0]) + str(pred_target_point[s][0][1] + tar_offset_pred[s][1])
                        title = 'EGO: ' + str(pred_target_point[s][0]) + 'ATT: ' + str(pred_target_point[s][0] + tar_offset_pred[s])
        ...
        # if fail, iou distribution
        bins = np.linspace(0, 1, num=100)
        hist, bins = np.histogram(ego_iou_all_distribution, bins=bins, density=False)
        plt.bar(bins[:-1], hist, align='center', width=0.02)
        plt.xlabel('Ego IOU')
        plt.ylabel('Freq')
        plt.title('Ego IOU distribution when failed')
        # plt.show()
        plt.savefig(save_folder + '/Ego IOU distribution when failed.png')
        plt.close()

        # if fail, ttc distribution
        ...
        plt.ylabel('Freq')
        plt.title('TTC distribution when failed')
        # plt.show()
        plt.savefig(save_folder + '/TTC distribution when failed.png')
        plt.close()

        # if fail, ttc distribution
        bins = np.linspace(0, 1, num=100)
        hist, bins = np.histogram(pred_right_lane_distribution, bins=bins, density=False)
        plt.bar(bins[:-1], hist, align='center', width=0.02)
        plt.xlabel('Correct lane id ratio')
        plt.ylabel('Freq')
        plt.title('Correct lane id ratio distribution when failed')
        # plt.show()
        ...
        if plot_detailed_distribution_when_failed:
            ...
            all_type_ego_iou_distribution_list = [ego_iou_junction_crossing, ego_iou_LTAP, ego_iou_lane_change, ego_iou_opposite_direction, ego_iou_rear_end]
            all_type_ttc_distribution_list = [ttc_junction_crossing, ttc_LTAP, ttc_lane_change, ttc_opposite_direction, ttc_rear_end]
            all_type_pred_right_lane_distribution_list = [pred_right_lane_JC, pred_right_lane_LTAP, pred_right_lane_LC, pred_right_lane_OD, pred_right_lane_RE]
            if not os.path.isdir(save_folder + '/failed_analysis/EGO_IOU'):
                os.mkdir(save_folder + '/failed_analysis/EGO_IOU')
            for type_failed_index in range(len(all_type_ego_iou_distribution_list)):
                now_bins = np.linspace(0, 1, num=100)
                hist, now_bins = np.histogram(all_type_ego_iou_distribution_list[type_failed_index], bins=now_bins, density=False)
                plt.bar(now_bins[:-1], hist, align='center', width=0.02)
                plt.xlabel('Ego IOU')
                plt.ylabel('Freq')
                plt.title('Ego IOU distribution when failed')
                # plt.show()
                plt.savefig(save_folder + '/failed_analysis/EGO_IOU/' + now_type_list[type_failed_index] + ' Ego IOU distribution when failed.png')
                plt.close()
            if not os.path.isdir(save_folder + '/failed_analysis/TTC'):
                os.mkdir(save_folder + '/failed_analysis/TTC')
            for type_failed_index in range(len(all_type_ttc_distribution_list)):
                now_bins = np.linspace(min(all_type_ttc_distribution_list[type_failed_index]), max(all_type_ttc_distribution_list[type_failed_index]), num=50)
                hist, now_bins = np.histogram(all_type_ttc_distribution_list[type_failed_index], bins=now_bins, density=False)
                plt.bar(now_bins[:-1], hist, align='center', width=0.8)
                plt.xlabel('TTC (frame)')
                plt.ylabel('Freq')
                plt.title('TTC distribution when failed')
                # plt.show()
                plt.savefig(save_folder + '/failed_analysis/TTC/' + now_type_list[type_failed_index] + ' TTC distribution when failed.png')
                plt.close()
            if not os.path.isdir(save_folder + '/failed_analysis/Lane_id'):
                os.mkdir(save_folder + '/failed_analysis/Lane_id')
            for type_failed_index in range(len(all_type_pred_right_lane_distribution_list)):
                now_bins = np.linspace(0, 1, num=100)
                hist, now_bins = np.histogram(all_type_pred_right_lane_distribution_list[type_failed_index], bins=now_bins, density=False)
                plt.bar(now_bins[:-1], hist, align='center', width=0.02)
                plt.xlabel('Correct lane id ratio')
                plt.ylabel('Freq')
                plt.title('Correct lane id ratio distribution when failed')
                # plt.show()
                plt.savefig(save_folder + '/failed_analysis/Lane_id/' + now_type_list[type_failed_index] + ' Correct lane id ratio distribution when failed.png')
                plt.close()
        ...
        ttc_averages = {ttc: np.mean(scene_list) for ttc, scene_list in ttc_with_yaw_dict.items()}
        for ttc, avg_value in ttc_averages.items():
            print(f"TTC {ttc}: Average Value = {avg_value:.2f}")
        ...
        print("Right GT collision rate:", attacker_right_collision_rate / all_data_num, attacker_right_collision_rate, all_data_num)
        print("Collide with other:", col_other_num / all_data_num)
        ...
        csv_file = 'run/' + split + '_performance_F1.csv'
        if not os.path.exists(csv_file):
            print("not exist")
            with open(csv_file, 'a+') as f:
                writer = csv.writer(f)
                # writer.writerow(['Time', 'LR', 'Adam_weight_decay', 'Ego iou>0.5', 'FDE', 'Ego yaw', 'TP iou>0.5', 'TP DE', 'TP yaw', 'TP ID', 'CR', 'Sim'])
                writer.writerow(['Time', 'LR', 'Adam_weight_decay', 'Positive_weight(RCNN)', 'Nearest_subgraph', 'Recall', 'Precision', 'F1', 'Ego FDE', 'Ego yaw']) # RCNN version
                f.close()
        with open(csv_file, 'a+') as f:
            writer = csv.writer(f)
            ...
            writer.writerow([save_folder.split('/')[-1], self.lr, self.weight_decay, self.positive_weight, self.nearest_subgraph, recall, precision, F1_score, sum_fde / all_data_num, all_yaw_distance / all_data_num])  # RCNN version
            f.close()
        # all_metric_csv_directory = save_folder + '/all_metric_' + split + '_performance.csv'
        all_metric_csv_directory = 'run/all_module_' + split + '_performance.csv'
        if not os.path.exists(all_metric_csv_directory):
            ...
            with open(all_metric_csv_directory, 'a+') as all_metric_f:
                ...
                all_metric_writer.writerow(['Time', 'LR', 'Adam_weight_decay', 'Positive_weight(RCNN)', 'Nearest_subgraph', 'Ego iou>0.5', 'FDE', 'Ego yaw', 'TP iou>0.5', 'TP DE', 'TP yaw', 'TP ID', 'CR', 'Sim'])
        ...
        with open(all_metric_csv_directory, 'a+') as all_metric_f:
            ...
            all_metric_writer = csv.writer(all_metric_f)
            all_metric_writer.writerow([save_folder.split('/')[-1], self.lr, self.weight_decay, self.positive_weight, self.nearest_subgraph, ego_iou_50 / all_data_num, sum_fde / all_data_num, all_yaw_distance / all_data_num, 
                             attacker_iou_50 / all_data_num, sum_attacker_de / all_data_num, ideal_yaw_dist_average / all_data_num,
                             all_tp_cls_acc / batch_num, cr, Average_Similarity])
            all_metric_f.close()

        csv_file = 'run/' + split + '_cr_sim.csv'
        if not os.path.exists(csv_file):
            with open(csv_file, 'a+') as f:
                writer = csv.writer(f)
                # writer.writerow(['Time', 'LR', 'Adam_weight_decay', 'Ego iou>0.5', 'FDE', 'Ego yaw', 'TP iou>0.5', 'TP DE', 'TP yaw', 'TP ID', 'CR', 'Sim'])
                writer.writerow(['Time', 'LR', 'Adam_weight_decay', 'Positive_weight(RCNN)', 'Nearest_subgraph', 'Type', 'CR', 'Sim(5)', 'Sim(10)', 'Sim(20)', 'Sim(30)', 'Yaw dist']) # RCNN version
                f.close()
        with open(csv_file, 'a+') as f:
            writer = csv.writer(f)
            # writer.writerow([save_folder.split('/')[-1], self.lr, self.weight_decay, ego_iou_50 / all_data_num, sum_fde / all_data_num, all_yaw_distance / all_data_num, 
            ...
            for type_index in range(len(type_name_list)):
                if type_index == len(type_name_list) - 1:
                    ...
                    writer.writerow([save_folder.split('/')[-1], self.lr, self.weight_decay, self.positive_weight, self.nearest_subgraph, type_name_list[type_index],
    ...
    def eval_when_training(self,
        ...
        pred_collision_rate = 0
        all_tp_cls_acc = 0
        attacker_right_collision_rate = 0.00000001
